...
Excluding folders, and every repository they contain, from a git-cleaner run with `--exclude` patterns relative to the scanned root; how exclusions are validated and reported; and the repeatable pattern flags `--keep` and `--exclude`.
